...
2. Default location:
   - If in git repo → Project-level: `.opencodecommands/`
   - If not in git repo → Global: `~/.config/opencode/commands/`
3. Allow user override:
   - If user explicitly mentions "global" or "user-level" → Use `~/.config/opencode/commands/`
   - If user explicitly mentions "project" or "project-level" → Use `.opencodecommands/`
...
1. Determine full file path:
   - Project: `.opencodecommands/[command-name].md`
   - Global: `~/.config/opencode/commands/[command-name].md`
